...
colnames(PCoA_Bio_wthrare)[1] <- "Annee"
PCoA_Bio_wthrare <- as.data.frame(PCoA_Bio_wthrare)
rownames(PCoA_Bio_wthrare) <- PCoA_Bio_wthrare$Annee
PCoA_Bio_wthrare$Annee <- NULL
head(PCoA_Bio_wthrare)
rownames(PCoA_Bio_wthrare)

# indice de dissimilarité de Bray
PCoA_Bio_Bray <- vegdist(PCoA_Bio_wthrare, method = "bray")
PCoA_Bio_Bray
str(PCoA_Bio_Bray)
...
corVar_b<-cor(res_PCoA_Bio_Bray$vectors, PCoA_Bio_wthrare)
...
seuil_contrib <- 0.60  # à ajuster si besoin
